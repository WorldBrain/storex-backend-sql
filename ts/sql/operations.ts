...
export interface OperationTransformOptions {
  ...
  getStoredForeignKeyName(sourceCollection: string, relationName: string): string;
  getStoredFieldName?(collectionName: string, fieldName: string): string | null;
}
...
export function transformOperationTemplate(
  ...
  const setValues = (target: { [key: string]: SqlValueNode | SqlPlaceholderNode }, collection: string, values: any) => {
    for (const [objectFieldName, fieldValue] of Object.entries(values)) {
      const fieldName = options?.getStoredFieldName?.(collection, objectFieldName) ?? objectFieldName;
      const [isPlaceholder, placeholder] = allowPlaceholder(context, fieldValue);
      target[fieldName] = isPlaceholder ? placeholder : { literal: fieldValue as any };
    }
  };
  const setWhere = (
    ...
    if (operation.where && Object.keys(operation.where).length) {
      target.where = transformOperationWhere(operation.where, context, options);
    }
  };
  ...
  } else if (operation.operation === "findObject" || operation.operation === "findObjects") {
...
function getSelectFields(
  ...
  const pushFields = (collection: string, tableAlias: string, fieldNames: string[]) => {
    for (const fieldName of fieldNames) {
      const storedFieldName = options.getStoredFieldName?.(collection, fieldName) ?? fieldName;
      fields.push({
        source: {
  ...
  pushFields(operation.collection, operation.collection, options.getFieldNames(operation.collection));

  const pushRelationFields = (relation: QueryRelation) => {
    if (!(relation.fetch ?? true)) {
      return;
    }
    pushFields(relation.relation, relation.alias ?? relation.relation, options.getFieldNames(relation.relation));
...
function getSelectJoins(
  ...
  const pushJoin = (source: string, relation: QueryRelation) => {
    ...
    const foreignKeyName = options.getStoredForeignKeyName(source, relation.relation);
    const pkField = options.getPkField(source);
    const storedPkField = options.getStoredFieldName?.(source, pkField) ?? pkField;

    const foreingKeyEquals: SqlEqualNode = {
      eq: [
        { source: { tableName: { identifier: alias }, fieldName: { identifier: foreignKeyName } } },
        { source: { tableName: { identifier: source }, fieldName: { identifier: storedPkField } } },
      ],
    };
...
function transformOperationWhere(
  ...
  const getSourceFromShorthand = (shorthand: string): SqlSource => {
    ...
    return {
      tableName: options?.hasRelations ? { identifier: options.collectionName } : undefined,
  ...
  if (isPlainObject(whereNode)) {
    ...
    if (keys.length === 1) {
        ...
        if (childKeys.length === 1) {
          ...
          const transformed: SqlBinaryOp<any> = {
            [op]: [
              ...
              transformOperationWhere(childValue, context, options) as any,
            ],
          };
      ...
      return {
        and: Object.entries(whereNode).map(
          ([key, value]: any) => transformOperationWhere({ [key]: value }, context, options) as any
        ),
      };
    }
    ...
    if (parts.length <= 2) {
      const sourceNode: SqlSourceNode = {
        source: {
          tableName: parts.length === 2 ? { identifier: parts[0] } : undefined,
          fieldName: { identifier: parts.length === 2 ? parts[1] : parts[0] },
        },
      };
  ...
  throw new Error(`Can't transform operation where: ${JSON.stringify(whereNode)}`);
}

function allowPlaceholder(context: OperationTransformContext, node: any): [false, any] | [true, SqlPlaceholderNode] {
...
function transformPlaceholder(context: OperationTransformContext, node: any): SqlPlaceholderNode {

...
function indentRenderableLines(lines: SqlRenderableLine[]): SqlRenderableLine[] {
  return lines.map((line) => [line[0] + 4, line[1]]);
}

export function renderSqlAst(params: { ast: astTypes.SqlAst; nodes: SqlRenderNodes }) {
  return params.ast.map((node) => renderSqlNodeAsString(node, params.nodes)).join("\n");
}
...
export function renderSqlNode(node: any, nodes: SqlRenderNodes): SqlRenderableLine[] {
  // console.log(node)
...
export const renderInsertNode: SqlRenderNode<astTypes.SqlInsertNode> = (node, context) => {
  const tableName = context.renderNodeAsString(node.insert.tableName);
  const fieldNames = Object.keys(node.insert.values);
  const fieldList = fieldNames.map((fieldName) => context.renderNodeAsString({ identifier: fieldName })).join(", ");
  const valuesList = fieldNames
    .map((fieldName) => context.renderNodeAsString(node.insert.values[fieldName]))
    .join(`, `);
  return [[0, `INSERT INTO ${tableName} (${fieldList}) VALUES (${valuesList});`]];
};

export const renderSelectNode: SqlRenderNode<astTypes.SqlSelectNode> = (node, context) => {
  const fieldList = node.select.fields.map((fieldNode) => {
    const sourceNode: astTypes.SqlSourceNode = { source: { ...fieldNode.source, aliasType: "as" } };
  ...
  const query = maybeWithWhere([[0, content]], node.select, context, { dontTerminate: true });
...
export const renderUpdateNode: SqlRenderNode<astTypes.SqlUpdateNode> = (node, context) => {
  const tableName = context.renderNodeAsString(node.update.tableName);
  const assingments = Object.entries(node.update.updates)
    .map(
      ([fieldName, fieldValue]) =>
        context.renderNodeAsString({ identifier: fieldName }) + " = " + context.renderNodeAsString(fieldValue)
    )
    .join(", ");
...
export const renderDeleteNode: SqlRenderNode<astTypes.SqlDeleteNode> = (node, context) => {
...
export const renderSourceNode: SqlRenderNode<astTypes.SqlSourceNode> = (node, context) => {
  const { source } = node;
  let fieldStr = source.fieldName ? context.renderNodeAsString(source.fieldName) : ``;
  if (source.tableName) {
...
export const renderJoinNode: SqlRenderNode<astTypes.SqlJoinNode> = (node, context) => {
...
function maybeWithWhere(
  ...
  if (nodeContent.where) {
    lines[0][1] += " WHERE";
    lines.push(...indentRenderableLines(context.renderNode(nodeContent.where)));
  }
...
export const renderTestIdentifierNode: SqlRenderNode<astTypes.SqlIdentifierNode> = (node) => {
  return [[0, node.identifier]];
};

export const renderTestLiteralNode: SqlRenderNode<astTypes.SqlLiteralNode> = (node) => {
  let result: string;
...
export const renderDateTimeNode: SqlRenderNode<astTypes.SqlDateTimeNode> = (node) => {
...
function binaryNode(key: string, sqlOp: string) {
  const renderNode: SqlRenderNode = (node, context) => {
    const content = node[key].map((operand: any) => context.renderNodeAsString(operand)).join(` ${sqlOp} `);
    return [[0, `(${content})`]];
  };
  return renderNode;
}

export const renderCreateTableNode: SqlRenderNode<astTypes.SqlCreateTableNode> = (node, context) => {
  ...
  for (const [fieldIndex, [fieldName, fieldDefinition]] of node.createTable.fields.entries()) {
    const isLastLine = fieldIndex + 1 >= numItems;
    const tralingComma = !isLastLine ? "," : "";
    const flags = fieldDefinition.flags.join(" ");
    lines.push([0, `${fieldName} ${fieldDefinition.type} ${flags}${tralingComma}`]);
  }
  for (const [fkIndex, fkNode] of (node.createTable.foreignKeys ?? []).entries()) {
  ...
  return [[0, `CREATE TABLE ${node.createTable.tableName} (`], ...indentRenderableLines(lines), [0, `);`]];
};

export const renderForeignKeyNode = (options: { withConstraint: boolean }) => {
  const render: SqlRenderNode<astTypes.SqlForeinKeyNode> = (node, context) => {
    ...
    const renderableLines: SqlRenderableLine[] = lines.map((line) => [0, line]);
    return [renderableLines[0], ...indentRenderableLines(renderableLines.slice(1))];
  };
  return render;
};

export function isSqlNodeType<T, U extends keyof T>(node: any, type: U): node is T {
  return isPlainObject(node) && type in node;
}

export const DEFAULT_SQL_NODES: Omit<SqlRenderNodes, "literal" | "placeholder" | "identifier"> = {

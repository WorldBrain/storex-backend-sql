...
export function getReverseRelationDefinitions(
    ...
    for (const [collectionName, collectionDefinition] of Object.entries(
        storageCollectionDefinitions,
    )) {
        for (const relation of collectionDefinition.relationships ?? []) {
            if (
                'childOf' in relation &&
                relation.childOf === targetCollectionName
            ) {
                ...
                'singleChildOf' in relation &&
                relation.singleChildOf === targetCollectionName
            ) {
                addRelation(collectionName, {
                    collectionName,
                    relation: relation,
...
export function getRelationAlias(
    ...
    if ('childOf' in relation || 'singleChildOf' in relation) {
        const alias =
            direction === RelationDirection.Origin
                ? relation.alias
                : relation.reverseAlias
        if (!alias) {
            if (direction === RelationDirection.Origin) {
                return 'childOf' in relation
                    ? relation.childOf
        ...
        return (
            relation.aliases?.[pointer.connectionIndex!] ??
            relation.connects[pointer.connectionIndex!]
        )
    } else {
...
export function isConnectsRelation(
    relation: Relationship,
): relation is ConnectsRelationship {
    return 'connects' in relation
}

export function isConnectionPointer(
    pointer: RelationPointer,
): pointer is ConnectionRelationPointer {
...
export function getPkField(collectionDefinition: CollectionDefinition) {
    for (const index of collectionDefinition.indices ?? []) {
        if (index.pk) {
            ...
            throw new Error(
                `Compound primary keys are not implemented in SQL backend (yet)`,
            )
        }
    }
    return 'id'
...
export function isoToDate(isoString: string) {
    ...
    const dateString = time ? `${date}T${time}Z` : date

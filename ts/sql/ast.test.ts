...
export function test(params: { ast: SqlAst; expected: string; nodes?: Partial<SqlRenderNodes> }) {
...
describe("SQL AST", () => {
  ...
  it("should render SELECT", () => {
    test({
      ast: [
        {
          select: {
            fields: [{ source: { fieldName: { identifier: "fieldStr" } } }],
            source: { tableName: { identifier: "myTable" } },
            where: {
              and: [
                { eq: [{ source: { fieldName: { identifier: "fieldInt" } } }, { literal: 5 }] },
                { eq: [{ source: { fieldName: { identifier: "fieldStr" } } }, { literal: "foo" }] },
              ],
            },
          },
        },
  ...
  it("should SELECT with JOIN", () => {
    test({
      ast: [
        {
          select: {
            fields: [{ source: { fieldName: { identifier: "fieldStr" }, alias: { identifier: "fStr" } } }],
            source: { tableName: { identifier: "firstTable" }, alias: { identifier: "t1" } },
            where: {
              eq: [{ source: { fieldName: { identifier: "fieldInt" } } }, { literal: 5 }],
            },
            joins: [
              {
                ...
                where: {
                  eq: [
                    { source: { tableName: { identifier: "t2" }, fieldName: { identifier: "firstTableId" } } },
                    { source: { tableName: { identifier: "t1" }, fieldName: { identifier: "id" } } },
                  ],
                },
              },
              {
                ...
                where: {
                  eq: [
                    { source: { tableName: { identifier: "t3" }, fieldName: { identifier: "secondTableId" } } },
                    { source: { tableName: { identifier: "t2" }, fieldName: { identifier: "id" } } },
                  ],
                },
              },
            ],
  ...
  it("should render placeholders", () => {
    const testPlaceholder = (expectedPlaceholder: string, node: SqlRenderNode<SqlPlaceholderNode>) =>
      test({
        ast: [
          {
            select: {
              ...
              where: {
                eq: [
                  ...
                  { placeholder: { position: 1, name: "fieldInt" } },
                ],
              },
            },
          },
    ...
    testPlaceholder(":fieldInt", (node) => [[0, `:${node.placeholder.name}`]]); // SQLite
  });
      ...
      ast: [
        {
          update: {
            tableName: { identifier: "myTable" },
            where: {
              eq: [{ source: { fieldName: { identifier: "fieldInt" } } }, { literal: 1 }],
            },
            updates: {
              fieldStr: { literal: "test" },
            },
      ...
      ast: [
        {
          delete: {
            tableName: { identifier: "myTable" },
            where: { eq: [{ source: { fieldName: { identifier: "id" } } }, { literal: 1 }] },
          },
        },
      ],
      expected: `
  ...
  it(`should render CREATE TABLE`, () => {
    const testCreateTable = (foreignKey: ReturnType<typeof renderForeignKeyNode>, expected: string) =>
      test({
        nodes: {
          foreignKey,
        },
        ast: [
          {
            createTable: {
              ...
              fields: [
                ...
                ["id", { type: "SERIAL", flags: ["PRIMARY KEY"] }], // PostgreSQL
                ["fieldStr", { type: "TEXT", flags: ["NOT NULL"] }],
                ["parentId", { type: "INTEGER", flags: ["NOT NULL"] }],
              ],
              foreignKeys: [
                {
                  foreignKey: {
